...
const Applicationform = () => {
    // state para sa current step
    const [currentStep, setCurrentStep] = useState(1);


    // Data para sa stepper
    const steps = [
            ...
            <div className="w-full md:w-64 flex-shrink-0 sticky top-32 self-start mt-4">
              {steps.map((step, index) => (
                <div key={step.id} className="flex items-center justify-end mb-10 relative">
                  ...
                  {/* Step Title */}
                  <span className={`mr-4 text-[10px] md:text-xs font-bold tracking-widest text-right max-w-[150px] ${currentStep >= step.id ? 'text-slate-900' : 'text-slate-400'}`}>
                    {step.title}
                  </span>
                  ...
                  <div className="relative z-10">
                    {currentStep > step.id ? (
                      ...
                      <div className={`w-9 h-9 rounded-lg border-2 flex items-center justify-center font-bold transition-all ${currentStep === step.id ? 'border-[#0056b3] text-[#0056b3] bg-white shadow-md' : 'border-slate-300 text-slate-400 bg-white'}`}>
                        {step.id}
                      </div>
                    )}
                  </div>
                  ...
                  {index !== steps.length - 1 && (
                    <div className={`absolute right-[17px] top-9 w-[1.5px] h-10 -z-0 ${currentStep > step.id ? 'bg-green-500' : 'bg-slate-300'}`} />
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="border-l-[1.5px] border-slate-300 pl-10 md:pl-10 h-full">
                ...
                {currentStep === 1 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <p className="text-slate-500">Ito ang section para sa Personal Information...</p>
                  </div>
                )}
  
                {/* Step 3: Eligibility (Gaya ng sa screenshot mo) */}
                {currentStep === 3 && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                    <div className="bg-slate-200 h-14 w-full md:w-3/4 rounded-lg"></div>
                    <p className="text-slate-500 italic">Please provide your eligibility details here.</p>
                  </div>
                )}
